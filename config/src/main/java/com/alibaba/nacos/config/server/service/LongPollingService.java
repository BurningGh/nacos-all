...
public class LongPollingService extends AbstractEventListener {
    ...
    public void addLongPollingClient(HttpServletRequest req, HttpServletResponse rsp, Map<String, String> clientMd5Map,
        ...
        if (isFixedPolling()) {
            ...
            // dengwj3 根据tag或者是否是vip从缓存中取出服务器保存的对应的md5进行比较，如果有更新则立即返回（resp设置不可缓存）
            ...
            if (changedGroups.size() > 0) {
                ...
                // dengwj3 无更新时，如果请求设置了不挂起，则立马返回为空
        ...
        // dengwj3 如果无变化，请求也没设置立马返回，则调度执行ClientLongPolling的job，超时时间到之前执行再次检查是否否变化
        scheduler.execute(
    ...
    public void onEvent(Event event) {
        if (isFixedPolling()) {
            ...
            // 如果是本地文件改变了，发布LocalDataChangeEvent事件，通知监听的客户端，改变的配置文件名，不用等待延迟10s的回复
    ...
    // dengwj3 如果某个配置文件更新了，发布事件这个task将启动，检查beta和tag后从订阅客户端中迭代包含了该配置文件的进行返回
