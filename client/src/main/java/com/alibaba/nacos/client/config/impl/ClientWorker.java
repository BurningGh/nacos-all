...
public class ClientWorker implements Closeable {
    ...
    // dengwj3  检查缓存与配置是否一直，不一致则用本地配置更新缓存
    ...
    public void checkConfigInfo() {
        ...
        // dengwj3 一批任务最大3000一批，当超过3000时会新建另一个任务
     ...
     * // dengwj3 检查哪些监听的dateIds改变了，应该是根据md5
            ...
            try {
                ...
                for (CacheData cacheData : cacheMap.get().values()) {
                    if (cacheData.getTaskId() == taskId) {
                        ...
                        try {
                            // dengwj3 将本地配置文件，同步到缓存中
                            checkLocalConfig(cacheData);
                            if (cacheData.isUseLocalConfigInfo()) {
                                // dengwj3 检查listener的md5和cacheData的不一致则进行通知listener
                ...
                // check server config
                // dengwj3 从服务器上获取到已经更新了的dataId列表，服务器根据md5判断是否已经更新
                ...
                for (String groupKey : changedGroupKeys) {
                    ...
                    try {
                        // dengwj3 发现更新了的dataid遍历进行从服务器获取最新的配置，拿到配置后更新缓存
                        String[] ct = getServerConfig(dataId, group, tenant, 3000L);
                ...
                // dengwj3 客户端执行完获取变化的配置id再拿id去获取新配置，后又重新执行改任务，如果本次任务异常则会延迟一段时间继续执行
